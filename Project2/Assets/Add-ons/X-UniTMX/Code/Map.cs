...
namespace X_UniTMX
{
	...
	public class Map
	{
		...
		/// <param name="onMapFinishedLoading">Callback for when map xml finishes loading</param>
		public Map(	string wwwPath, Action<Map> onMapFinishedLoading = null)
		{
		...
		Action<object, Action<Layer>> OnFinishedGeneratingTileSets = null;
		...
		private void GenerateTileSetsMaterials()
		{
			...
			if (OnFinishedGeneratingTileSets != null)
				OnFinishedGeneratingTileSets(OnFinishedGeneratingTileSetsReturnObject, OnGeneratedLayer);
		...
		private void GenerateTileSets(Action<object, Action<Layer>> onFinishedGeneratingTileSets, object obj = null, Action<Layer> onGeneratedLayer = null)
		{
			Tiles.Clear();
			_tileSetsToLoaded = 0;
		...
		public void GenerateTileLayers(Action onGeneratedTileLayers = null, Material baseTileMaterial = null)
		{
				...
				foreach (var layerPair in namedLayers)
				{
					if (layerPair.Value is TileLayer)
					{
						...
						if (HasGeneratedTileSets)
							tileLayer.Generate(TileSetsMaterials, OnTileLayerGenerated);
						else
							GenerateTileSets(ContinueGenerateTileLayer, tileLayer, OnTileLayerGenerated);
					}
				}
			}
		}
		...
		public void GenerateTileLayer(string layerName, Action<Layer> onGeneratedTileLayer = null, Material baseTileMaterial = null)
		{
			if (BaseTileMaterial == null)
				BaseTileMaterial = baseTileMaterial;
			if (BaseTileMaterial == null)
			...
			if (tileLayer != null)
			{
				tileLayer.MakeUniqueTiles = (tileLayer.MakeUniqueTiles || GlobalMakeUniqueTiles);
				if (HasGeneratedTileSets)
					tileLayer.Generate(TileSetsMaterials, onGeneratedTileLayer);
				else
					GenerateTileSets(ContinueGenerateTileLayer, tileLayer, onGeneratedTileLayer);
			}
		}

		protected void ContinueGenerateTileLayer(object tileLayer, Action<Layer> onGeneratedLayer)
		{
			(tileLayer as TileLayer).Generate(TileSetsMaterials, onGeneratedLayer);
		}
		#endregion
		...
		public void GenerateImageLayers(Action onGeneratedImageLayers = null, Material baseTileMaterial = null)
		{
			if (BaseTileMaterial == null)
				BaseTileMaterial = baseTileMaterial;
			if (BaseTileMaterial == null)
			...
			else
			{
				foreach (var layerPair in namedLayers)
				{
					...
					{
						(layerPair.Value as ImageLayer).Generate(BaseTileMaterial, OnImageLayerGenerated);
					}
				}
			}
		}
		...
		public void GenerateImageLayer(string layerName, Action<Layer> onGeneratedImageLayer = null, Material baseTileMaterial = null)
		{
			if (BaseTileMaterial == null)
				BaseTileMaterial = baseTileMaterial;
			if (BaseTileMaterial == null)
			...
			{
				if (HasGeneratedTileSets)
					imageLayer.Generate(BaseTileMaterial, onGeneratedImageLayer);
			}
		}
		#endregion
		...
		public void GenerateMapObjectLayers(Action onGeneratedMapObjectLayers = null, Material baseTileMaterial = null)
		{
			if (BaseTileMaterial == null)
				BaseTileMaterial = baseTileMaterial;
			if (BaseTileMaterial == null)
			...
			else
			{
				foreach (var layerPair in namedLayers)
				{
					...
					{
						if (HasGeneratedTileSets)
							(layerPair.Value as MapObjectLayer).Generate(TileSetsMaterials, OnMapObjectLayerGenerated);
						else
							GenerateTileSets(ContinueGenerateMapObjectLayer, (layerPair.Value as MapObjectLayer), OnMapObjectLayerGenerated);
					}
				}
			}
		}
		...
		public void GenerateMapObjectLayer(string layerName, Action<Layer> onGeneratedMapObjectLayer = null, Material baseTileMaterial = null)
		{
			if (BaseTileMaterial == null)
				BaseTileMaterial = baseTileMaterial;
			if (BaseTileMaterial == null)
			...
			{
				if (HasGeneratedTileSets)
					objectLayer.Generate(TileSetsMaterials, onGeneratedMapObjectLayer);
				else
					GenerateTileSets(ContinueGenerateMapObjectLayer, objectLayer, onGeneratedMapObjectLayer);
			}
		}

		protected void ContinueGenerateMapObjectLayer(object mapObjectLayer, Action<Layer> onGeneratedLayer)
		{
			(mapObjectLayer as MapObjectLayer).Generate(TileSetsMaterials, onGeneratedLayer);
		}
		#endregion

		#region Generic Generate Map
		
		public void Generate(GameObject parent, Material baseTileMaterial, int sortingOrder = 0, bool makeUnique = false, Action<Map> onFinishedGeneratingMap = null)
		{
		...
		{
			_calls++;
			// This function will be called once from each Generate___Layers function :P
			if (_calls >= 3)
			{
		...
		public void GenerateColliders(bool is2DCollider, bool collidersAreTrigger, float collidersZDepth, float collidersWidth, bool collidersAreInner)
		{
			foreach (var layerPair in namedLayers)
			{
				if (layerPair.Value is MapObjectLayer)
				{
					GenerateCollidersFromLayer(layerPair.Value as MapObjectLayer, is2DCollider, collidersAreTrigger, collidersZDepth, collidersWidth, collidersAreInner);
				}
			}
		}

		public void GeneratePrefabs(bool is2DCollider, bool addMapName)
		{
			foreach (var layerPair in namedLayers)
			{
				if (layerPair.Value is MapObjectLayer)
				{
					GeneratePrefabsFromLayer(layerPair.Value as MapObjectLayer, is2DCollider, addMapName);
				}
			}
		}
		#endregion
					...
					for (int x = 0; x < tileLayer.Tiles.Width; x++)
					{
						for (int y = 0; y < tileLayer.Tiles.Height; y++)
						{
							...
							if (t.TileSet.TilesObjects.ContainsKey(t.OriginalID))
							{
								...
								{
									pathsList.Add(tileObj.GetPath(x, y, t.SpriteEffects, tileObjectEllipsePrecision));
								}
							}
						}
					}
		...
		{
			MapObjectLayer objectLayer = GetObjectLayer(objectLayerName);
			if (objectLayer != null) 
			{
				return GenerateCollidersFromLayer(objectLayer, is2DCollider, collidersAreTrigger, collidersZDepth, collidersWidth, collidersAreInner);
			}
			else
			{
		...
		/// <param name="collidersAreInner">true to generate inner collisions for 3D mapObjects</param>
		/// <returns>An Array containing all generated GameObjects</returns>
		public GameObject[] GenerateCollidersFromLayer(MapObjectLayer objectLayer, bool is2DCollider = true, bool collidersAreTrigger = false, float collidersZDepth = 0, float collidersWidth = 1, bool collidersAreInner = false)
		{
			if (objectLayer != null)
			{
				...
				foreach (MapObject colliderObjMap in colliders)
				{
					...
					//GeneratePrefab(colliderObjMap, newColliderObject, is2DCollider);

					// if this colider has transfer, so delete this colider
					//if (mapObj.GetPropertyAsBoolean("destroy collider after instantiate prefabs"))
					//{
		...
		/// <param name="is2DCollider">true to generate 2D colliders for the prefabs, if applicable</param>
		...
		public GameObject[] GeneratePrefabsFromLayer(MapObjectLayer objectLayer, bool is2DCollider = true, bool addMapName = false)
		{
			if (objectLayer != null)
			{
				...
				{
					generatedPrefabs.Add(GeneratePrefab(mapObj, objectLayer.LayerGameObject, is2DCollider, addMapName));
				}
		...
		/// <param name="addMapName">true to add Map's name to generated prefabs</param>
		/// <returns>An array containing all generated Prebafs</returns>
		public GameObject[] GeneratePrefabsFromLayer(string objectLayerName, bool is2DCollider = true, bool addMapName = false)
		{

			MapObjectLayer objectLayer = GetObjectLayer(objectLayerName);
			if (objectLayer != null)
			{
				return GeneratePrefabsFromLayer(objectLayer, is2DCollider, addMapName);
			}
			else
			{
				Debug.LogWarning("There's no Layer \"" + objectLayerName + "\" in tile map.");
		...
		/// <param name="use2DColliders">true to generate 2D colliders, if applicable</param>
		/// <returns>Generated GameObject from the Prefab.</returns>
		public GameObject GeneratePrefab(MapObject obj, GameObject parent = null, bool use2DColliders = false, bool addTileName = true)
		{
			if (obj.HasProperty(Property_PrefabName))
			{
				string prefabName = obj.GetPropertyAsString(Property_PrefabName);
				string baseResourcePath = obj.GetPropertyAsString(Property_PrefabPath);
				UnityEngine.Object resourceObject = Resources.Load(baseResourcePath + prefabName);
				Resources.UnloadUnusedAssets();
				if (resourceObject != null)
				{
					float zDepth = obj.GetPropertyAsFloat(Property_PrefabZDepth);
					...
					// copy coliders from parent
					// only copy if type of this object is diferent of "NoCollider"
					if (obj.Type.Equals(Object_Type_NoCollider) == false)
					{
						if (obj.GetPropertyAsBoolean(Property_PrefabAddCollider))
						{
							//CopyCollider(obj, ref parent, ref newPrefab, use2DColliders);
							AddCollider(newPrefab, obj, obj.Type.Equals(Object_Type_Trigger), use2DColliders, zDepth);
						}
						else
		...
		public void ApplyCustomProperties(GameObject gameObject, MapObject obj)
		{
			...
			{
				UnityEngineInternal.APIUpdaterRuntimeServices.AddComponent(gameObject, "Assets/Add-ons/X-UniTMX/Code/Map.cs (2816,5)", obj.GetPropertyAsString(Property_AddComponent + c));
				c++;
			}
			c = 1;
