...
namespace X_UniTMX
{
	...
	public class TiledMapComponentEditor : Editor
	{
		...
		private void ReadPropertiesAndVariables()
		{
			...
			if (_tiledMapComponent.TiledMap == null || _changedMap)
			{
				...
				)
			{
				...
				if (rebuildArrays || _tiledMapComponent.objectLayers == null)
					_tiledMapComponent.objectLayers = objectLayers.ToArray();
				if (rebuildArrays || _tiledMapComponent.generateCollider == null)
					_tiledMapComponent.generateCollider = generateCollider.ToArray();
				if (rebuildArrays || _tiledMapComponent.collidersIs2D == null)
					_tiledMapComponent.collidersIs2D = collidersIs2D.ToArray();
				if (rebuildArrays || _tiledMapComponent.collidersWidth == null)
					_tiledMapComponent.collidersWidth = collidersWidth.ToArray();
				if (rebuildArrays || _tiledMapComponent.collidersZDepth == null)
					_tiledMapComponent.collidersZDepth = collidersZDepth.ToArray();
				if (rebuildArrays || _tiledMapComponent.collidersIsInner == null)
					_tiledMapComponent.collidersIsInner = collidersIsInner.ToArray();
				if (rebuildArrays || _tiledMapComponent.collidersIsTrigger == null)
					_tiledMapComponent.collidersIsTrigger = collidersIsTrigger.ToArray();
				if (rebuildArrays || _tiledMapComponent.tileLayers == null)
					_tiledMapComponent.tileLayers = tileLayers.ToArray();
				if (rebuildArrays || _tiledMapComponent.imageLayers == null)
					_tiledMapComponent.imageLayers = imageLayers.ToArray();
				if (rebuildArrays || _tiledMapComponent.tileLayersFoldoutProperties == null)
					_tiledMapComponent.tileLayersFoldoutProperties = tileLayersFoldoutProperties.ToArray();
				if (rebuildArrays || _tiledMapComponent.objectLayersFoldoutProperties == null)
					_tiledMapComponent.objectLayersFoldoutProperties = objectLayersFoldoutProperties.ToArray();
				if (rebuildArrays || _tiledMapComponent.imageLayersFoldoutProperties == null)
					_tiledMapComponent.imageLayersFoldoutProperties = imageLayersFoldoutProperties.ToArray();
				if (rebuildArrays || _tiledMapComponent.MakeUniqueTiles == null)
		...
		void DoMapPropertiesGUI()
		{
			...
			{
				EditorGUILayout.SelectableLabel(string.Concat("Size (Tiles): ", _tiledMapComponent.TiledMap.Width.ToString(), "x", _tiledMapComponent.TiledMap.Height.ToString()));
				EditorGUILayout.SelectableLabel(string.Concat("Tile Size: ", _tiledMapComponent.TiledMap.TileWidth.ToString(), "x", _tiledMapComponent.TiledMap.TileHeight.ToString()));
				EditorGUILayout.SelectableLabel(string.Concat("Orientation: ", _tiledMapComponent.TiledMap.Orientation.ToString()));
				EditorGUILayout.SelectableLabel(string.Concat("Render Order: ", _tiledMapComponent.TiledMap.MapRenderOrder.ToString()));
		...
		void DoTileCollisionsGUI()
		{
			...
			if (foldoutTileCollisions.boolValue)
			{
				...
				TileCollisionsIs2D.boolValue = EditorGUILayout.Toggle("Create 2D mapObjects for tile collisions?", TileCollisionsIs2D.boolValue);
