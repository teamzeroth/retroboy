...
public class OldPLayer2 : MonoBehaviour {
    ...
    public IEnumerator SetAfterOnHurt() {
        yield return new WaitForSeconds(Game.PLAYER_INVULNERABILITY_TIME);
        afterOnHurt = _onHurt;
    }
    ...
    public void UpdateMove() {
        Vector2 deltaMovement = Vector2.zero;//controller.Update(this);
    ...
    public void OnGetHit(int damage, Vector2 direction, Collider2D other) {
        ...
        if (Life > 0) {
            ...
            OnHurt = true;
            StartFixedMove(direction.normalized * damage * 4, Vector2.zero, Game.PLAYER_HURT_TIME);
        } else {
            DisableColliders();
